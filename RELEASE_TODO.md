...
判定するコード (`src/lib/blocklist-match.ts`) が存在せず、HelloWork
取り込みバッチもこれを一切参照していなかった。admin が「これで今後
この語を含む求人は除外される」と信じて登録しても、実際には何も除外
されないサイレントな未実装状態だった。今回 `src/lib/blocklist-match.ts`
を実装し、`src/lib/crawler/import-batch.ts` の取り込みループに接続。
マッチした求人は取り込み対象から除外され、`Blocklist.hitCount` に
加算されるようになった。

- [ ] **重要**: この修正は今後の取り込みバッチにのみ適用される。既に
      登録済みのキーワードに一致する求人が過去の取り込みで既に DB に
      入っている場合、それらは自動では closed にならない（今回の修正は
      「今後取り込まない」だけで、既存データの遡及処理は行っていない）。
      既存の一致求人を洗い出して手動 close するか、別途バッチを用意する
      必要がないか運用側で確認してください。
- [ ] （任意の改善）キーワード登録時に「現在何件の active 求人が一致するか」
      のプレビューを表示する機能は未実装。単一文字や一般的すぎる語を
      登録すると大量の求人が意図せず除外され得るため、保存前にプレビュー
      できると安全。

---

## 🆕 admin「求人精査ダッシュボード」が rankScore を品質判定に流用している (2026-09-11 追加、要判断)

`/admin/job-quality` は `Job.rankScore` が低い求人を「低品質・要クローズ候補」
として一覧表示するが、`computeRankScore`（`src/lib/ranking.ts`）は検索結果の
並び順用に「品質シグナル」と「新着加点 (+15/+8)」「期限切れ間近ペナルティ
(-10/-20)」「人気度（閲覧数、鮮度で半減）」を全部まとめて 1 つの数値に
合成している。そのため:
- 内容は完全に問題ない求人でも、単に掲載から 30 日以上経って期限切れ間近
  というだけでスコアが下がり「低品質」バケットに落ちてしまう（false positive）
- 逆に `salaryMin: 1` のような形だけ埋めた求人や、内容が薄くても長さだけ
  ある説明文は品質シグナルを素通りし、上位バケットに紛れて見逃され得る
  （false negative）

admin は一覧の他の情報（タイトル・企業名等）も見た上でクローズ判断するため
致命的ではないが、ダッシュボードの前提（rankScore=品質）自体が実態と
ズレている。修正するには「検索順位用のスコア」と「内容の充実度だけを見る
品質スコア」を分離する必要があり、後者を高速に集計する方法（既存の
rankScore 列に相乗りするか、別途バッチ計算するか）は求人テーブルの規模を
見て判断すべきため、コードは変更せずここに記録するに留めた。

- [ ] rankScore とは独立した「品質スコア」を設計し、admin ダッシュボードは
      そちらを使うようにする（新着加点・期限切れペナルティ・人気度を含めない）
