...
[feature-decisions.md](./feature-decisions.md) で挙げた 3 つの矛盾は、以下で確定する：

### 矛盾 A: プッシュ通知 → ✅ やる

- 3.3 と 16.2 の重複。
- **16.2 を採用**（PWA 16.1 とセットで実装）。
- 3.x 通知設定 UI 上では「メール / LINE / プッシュ」の3チャネルから選べるようにする。
- 3.4 通知頻度・時間帯設定もプッシュに適用。

### 矛盾 B: LINE 公式アカウント連携 → ✅ やる（チャット応募は無し）

- 10.6 を採用、14.6 のチャット応募機能は持たない。
- LINE は **(a) Login (1.3)、(b) 通知配信 (3.2)、(c) リッチメニューからのサイト誘導 (10.6)** に絞る。
- `scripts/setup-line-rich-menu.ts` が既に存在するため流用。

### 矛盾 C: スカウト機能 → ❌ 持たない（一貫）

- 12.4 スカウトメール ❌ を優先、3.1 メール通知も「スカウト関連のみ」→「やらない」へ修正。
- ノンデスク産業ではスカウトより**求職者主導の応募**と**気になる（12.3）→企業からの接触**で十分。
- 企業から求職者への能動的アプローチは「**気になる返し**（求職者が 12.3 で気になるを押した相手に企業が「会いたい」ボタンを送る）」だけで成立させる。
