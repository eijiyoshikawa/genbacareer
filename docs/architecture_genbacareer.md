...
**建設業界特化型**求人ポータル。建築・土木・電気設備・内装・解体産廃・
施工管理・測量設計・ドライバー / 重機の「現場で働く人」のための
転職プラットフォーム。
...
| 軸 | 内容 |
|---|---|
| ターゲット業界 | **建設業界に特化** (建築/躯体/土木/電気設備/内装/解体産廃/施工管理/測量設計/ドライバー・重機) — `src/lib/categories.ts` の 9 カテゴリで固定運用 |
| ターゲットユーザー | スマホ中心の建設業界ワーカー(20〜50 代) |
| 差別化 | スワイプ UI + LINE 連携 + 採用決定ボーナス + 地域 × 職種 SEO |
| マネタイズ | **成果報酬型のみ**(採用成立時に手数料)。サブスクは持たない |
| 直接競合 | Indeed、施工管理求人ナビ、現キャリ、建設ジョブ |

> **業界拡張は行わない方針**: 介護・製造・飲食・小売・清掃・警備・農林水産等への
> 拡張は**しない**。 `src/lib/categories.ts` の `CATEGORIES` /
> `CONSTRUCTION_CATEGORY_VALUES` は建設業特化のまま固定。建設業内の
> サブカテゴリ追加・整理は適宜行う。
...
model Company {
  ...
  logoUrl         String?
  description     String?
  industry        String?  // 建設業内のサブ業界 (建築・土木・電気・内装 等)
  // 12.1 企業ページ
  slug            String   @unique
  // 12.6 ランキング
  rankScore       Float?
...
- 建設業界では**求職者主導の応募**と**気になる（12.3）→ 企業からの返し**で成立させる。
...
2. **業界分類**: 建設業界 9 カテゴリ (建築/躯体/土木/電気設備/内装/解体産廃/施工管理/測量設計/ドライバー・重機) で固定運用。他業界への拡張は行わない
3. **既存資産の活用**:
