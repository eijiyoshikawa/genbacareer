...
| 軸 | 内容 |
|---|---|
| Phase 1 ターゲット業界 | **建設業（建築/躯体/土木/電気設備/内装/解体産廃/施工管理/測量設計）+ ドライバー・重機** ※既存 `src/lib/categories.ts` の 9 カテゴリで MVP |
| Phase 2 以降の拡大 | 介護、製造、飲食、小売、清掃、警備、農林水産（順次） |
...
> **業界カテゴリ拡張時の注意**: 既存コードは `src/lib/categories.ts` の
> `CATEGORIES` / `CONSTRUCTION_CATEGORY_VALUES` / `isConstructionCategory` に
> 強く依存しているため、Phase 2 で業界を増やすときは
> (a) 新カテゴリを `CATEGORIES` に追加、(b) `CONSTRUCTION_CATEGORY_VALUES` を
> `NONDESK_CATEGORY_VALUES` のような汎用名にリネーム、(c) 全呼び出し箇所を更新、
> という順序で進める。
