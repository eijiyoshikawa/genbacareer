...
### Step 3: ブランディング監査
- `grep -ri "xwork" src/` 等で旧ブランド残りを検出
- ロゴ・タイトル・本文を「ゲンバキャリア」に統一（カタカナ表記必須）
- 業界分類（Industry enum）がノンデスク 9 業界（建設・運送・介護・製造・飲食・小売・清掃・警備・農林水産）に絞られているか確認

### Step 4: Phase 1 未実装機能の洗い出し
設計 architecture_genbacareer.md の Phase 1 リストと既存実装を突き合わせ、
未実装機能の優先度付き ToDo を作る。

### Step 5: 小さい単位で実装着手
ToDo を 1 機能ずつ実装。各機能ごとにコミット・プッシュ。
