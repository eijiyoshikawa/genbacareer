...
export default async function AdminDashboard() {
  ...
  // 並列実行: 重い count は WHERE 付きインデックスを使うように
  const [
    activeJobs,
    pendingJobsQuality,
    totalUsers,
    ...
    // 求人精査待ちは status=pending_review 等を採用していなければ 0
    prisma.job
      .count({ where: { status: "draft", source: "direct" } })
      .catch(() => 0),
    prisma.user.count().catch(() => 0),
    prisma.user.count({ where: { createdAt: { gte: since7d } } }).catch(() => 0),
      ...
      <section>
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard
            ...
            sub={pendingJobsQuality > 0 ? `下書き ${pendingJobsQuality} 件` : "全件公開中"}
          />
          <StatCard
      ...
      <section className="border bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          ...
          <TrendCell
            ...
            highlight
          />
          <TrendCell label="直近 7 日" value={newApplications7d.toLocaleString()} />
        </div>
    ...
    <div className="mt-4 space-y-2">
      <div>
        ...
        <div className="mt-1 h-5 w-full bg-gray-100">
          <div
            className="h-5 bg-primary-500"
            style={{ width: `${w30}%` }}
            aria-label={`30 日: ${total30} 件`}
          />
        </div>
      </div>
      <div>
        ...
        <div className="mt-1 h-5 w-full bg-gray-100">
          <div
            className="h-5 bg-amber-400"
            style={{ width: `${w7}%` }}
            aria-label={`7 日: ${total7} 件`}
          />
        </div>
      </div>
    </div>
