 ...
 *      → MoneyForward 連携失敗 (env 未設定 / API エラー)。放置すると永久に未回収になるため、
 *        pending と同様に「発行済をマーク」で手動発行 → 復旧できるようにする。
 *
...
export default async function AdminBillingTodoPage() {
  ...
  const [pending, invoiced, refundApproved, failed] = await Promise.all([
    ...
    // D. MoneyForward 連携失敗 → 手動発行での復旧待ち
    prisma.billingEvent.findMany({
      where: { status: "failed" },
      orderBy: { createdAt: "asc" },
      select: {
    ...
    <div>
      ...
      <section className="mt-10">
        <h2 className="text-lg font-bold text-gray-900">
          ...
          D. MoneyForward 連携失敗 — 要手動復旧 ({failed.length} 件)
        </h2>
        {failed.length === 0 ? (
          <div className="mt-3 border bg-warm-50 p-6 text-center text-sm text-gray-500">
            復旧待ちの失敗はありません。
