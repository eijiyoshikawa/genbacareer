 ...
 *      → MoneyForward 連携失敗等で請求書発行に失敗したもの。
 *        放置すると成果報酬が永久に請求されない (収益漏れ) ため、
 *        原因解消後に「再試行」で pending に戻して A のキューに合流させる。
 *
    ...
    <div>
      ...
      <section className="mt-10">
        <h2 className="text-lg font-bold text-gray-900">
          ...
          D. 請求書発行失敗 ({failed.length} 件)
        </h2>
        <p className="mt-1 text-xs text-gray-500">
          MoneyForward 連携エラー等で自動請求に失敗しました。原因を解消してから「再試行」を押すと A のキューに戻ります。
        </p>
        {failed.length === 0 ? (
          <div className="mt-3 border bg-warm-50 p-6 text-center text-sm text-gray-500">
            発行失敗はありません。
          ...
          <ul className="mt-3 space-y-2">
            {failed.map((r) => (
              <li key={r.id} className="border border-rose-200 bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-gray-900">
                      <Link
