 ...
 *      → MoneyForward 連携呼び出しが失敗し請求書未発行のまま放置されている成果報酬
 *      → 「再試行」で pending に戻し、A のキューに再度乗せる
 *
...
export default async function AdminBillingTodoPage() {
  ...
  return (
    <div>
      ...
      <section className="mt-10">
        <h2 className="text-lg font-bold text-gray-900">
          <AlertTriangle className="mr-2 inline-block h-5 w-5 text-rose-700" />
          D. 発行失敗 (要確認) ({failed.length} 件)
        </h2>
        <p className="mt-1 text-xs text-gray-500">
          MoneyForward 連携呼び出しが失敗し、請求書が未発行のまま残っている成果報酬です。
          原因を確認のうえ「再試行」で発行待ち (A) に戻してください。
        </p>
        {failed.length === 0 ? (
          <div className="mt-3 border bg-warm-50 p-6 text-center text-sm text-gray-500">
            発行失敗はありません。
          ...
          <ul className="mt-3 space-y-2">
            {failed.map((r) => (
              <li key={r.id} className="border border-rose-300 bg-rose-50 p-4 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-gray-900">
                      <Link
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <MarkBillingButton id={r.id} action="mark_retry" />
                  </div>
                </div>
              </li>
            ))}
