...
export async function GET(request: Request) {
  ...
  for (const c of companies) {
    ...
    // メール送信後にこの update が失敗すると、次回の cron 実行でも
    // planExpiryNotifiedAt が null のままなので同じ企業に再度メールが飛ぶ。
    // try/catch で囲み、この企業の失敗が残りの企業の処理を止めないようにする。
    try {
      ...
      console.error(`[cron/plan-expiry-notice] flag update failed for ${c.id}:`, e)
    }
  }
