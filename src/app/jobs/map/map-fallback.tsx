...
export function JobMapFallback({ points }: { points: MapPoint[] }) {
  ...
  return (
    <div className="space-y-6">
      <div className="border-2 border-dashed border-amber-300 bg-amber-50 dark:bg-amber-900/20 p-4 text-sm text-amber-800 dark:text-amber-200">
              ...
              return (
                <Link
                  ...
                  className={`flex items-center justify-between border px-3 py-2 text-sm transition ${
                    disabled
                      ? "border-gray-200 dark:border-gray-800 text-gray-400 cursor-default"
                      : "border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-200 hover:bg-primary-50 hover:border-primary-400 dark:hover:bg-primary-900/20"
                  }`}
                >
                  <span>{pref}</span>
                  <span className={`text-xs font-bold ${
                    count > 0 ? "text-primary-700 dark:text-primary-400" : "text-gray-400"
                  }`}>
                    {count}
                  </span>
                </Link>
