...
export function middleware(request: NextRequest) {
  ...
  // IP allowlist は /admin ページに加えて /api/admin/* (バッチ/管理 API) にも適用する。
  const adminIpProtectedRoutes = ["/admin", "/api/admin"]
  ...
  const isAdminAnyRoute = adminIpProtectedRoutes.some((r) => pathname.startsWith(r))

  // /admin/* と /api/admin/* への IP allowlist 制御。ADMIN_IP_ALLOWLIST 未設定なら無制限。
  // 設定済みなら /admin/login 含め全 /admin・/api/admin パスに適用 (ブルートフォース防御も兼ねる)。
  if (isAdminAnyRoute) {
