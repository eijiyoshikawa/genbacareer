...
/**
 * JSON-LD を <script> タグに dangerouslySetInnerHTML で埋め込むための安全なシリアライズ。
 * JSON.stringify はそのままでは "</script>" 等を含む文字列を実行可能な HTML として
 * リークさせてしまう（`<` をエスケープしないため）。DB 由来の値（求人タイトル・企業名等）
 * が含まれるため、必ずこの関数を通して埋め込むこと。
 */
