...
export const BLOCKED_OCCUPATION_PATTERN = new RegExp(
  [
    ...
    "カーキャリア",
    "霊柩",
    // 自動車整備・自動車板金（建築板金は対象内のため「板金」「鈑金」単独は入れない。
    // タイトルが「板金工」「鈑金工」だけの曖昧ケースは inferCategory 側で本文から判別）
    "自動車板金",
    "自動車鈑金",
...
export function inferCategory(
  ...
  // 「板金」「鈑金」(異体字、同義) は建築板金（屋根・外壁・雨樋・ダクト = 対象）と
  // 自動車板金（対象外）の両方があり、タイトルだけでは判別できないケースがある
  // （例: 「板金工」「鈑金工」）。タイトルに板金/鈑金を含む場合は、本文の自動車系
  // シグナルで対象外と判定する。
  if (/[板鈑]金/.test(titleLower)) {
    const automotive = /自動車|車両|車体|カー|バンパー|ディーラー|車検|純正部品|事故車/.test(text)
    const architectural = /建築[板鈑]金|屋根|外壁|雨樋|雨とい|ダクト|折板|瓦棒|葺き/.test(text)
