...
export const BLOCKED_OCCUPATION_PATTERN = new RegExp(
  [
    ...
    // 全角表記もタイトル正規化後にマッチする。複合語のみ列挙する。
    // NOTE: 「システム設計」は空調/給排水「システム設計」を、「客先常駐」は建設の
    // 客先常駐求人（建築施工管理 等）を巻き込むため意図的に除外している。
    // 製造オペレーター系も列挙しない（素の「オペレーター」は driver から外したので
    // 建設キーワードの無い製造オペレーターは自然に null になる。逆に列挙すると
    // 「建設機械オペレーター」を誤ブロックするため）。
    "システムエンジニア",
    "システム開発",
    "プログラマ",
    "ソフトウェア",
