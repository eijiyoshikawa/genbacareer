...
export async function fuzzySearchJobs(
  ...
  // Build params and WHERE clauses dynamically to keep positions consistent
  const sqlParams: unknown[] = [input.q, categories]
  let p = 3 // next param index
  const extraClauses: string[] = []

  if (input.prefecture) {
    extraClauses.push(`AND prefecture = $${p++}`)
    sqlParams.push(input.prefecture)
  }
  if (input.employmentType) {
    extraClauses.push(`AND employment_type = $${p++}`)
    sqlParams.push(input.employmentType)
  }
  if (input.source) {
    extraClauses.push(`AND source = $${p++}`)
    sqlParams.push(input.source)
  }
  if (input.publishedSince) {
    extraClauses.push(`AND published_at >= $${p++}`)
    sqlParams.push(input.publishedSince)
  }
  if (input.salaryMin) {
    extraClauses.push(`AND salary_min >= $${p++}`)
    sqlParams.push(input.salaryMin)
  }
  if (input.salaryMax) {
    extraClauses.push(`AND salary_max <= $${p++}`)
